 ...
 * @author Cory McKay
 */
public class FeatureProcessorForMFCC2D {
	/* FIELDS ***************************************************************** */
	 ...
	 *             invalid.
	 */
	public FeatureProcessorForMFCC2D(int window_size, double window_overlap,
		...
		window_overlap_offset = (int) (window_overlap * window_size);
...
//			aggContainer.add(aggList);
	 ...
	 */
	private double[] preProcessRecording(InputStream in, String name) throws Exception {
		...
		AudioSamples audio_data = new AudioSamples(new_stream, name, false);
	...
	private double[][][] getFeatures(double[] samples,
		...
		//TODO calculate second derivative  for mfcc values
		for (int feat = 0; feat < feature_extractors.length; feat++) {
			FeatureDefinition fd = feature_extractors[feat].getFeatureDefinition();
			if (fd.name.equals("MFCC")) {
				for (int win = 2; win < window_start_indices.length - 2; win++) {
					for (int i = 0; i < results[win][feat].length; i++) {
						results[win][feat][i] = 2* results[win + 2][feat][i] + results[win + 1][feat][i] - results[win - 1][feat][i] - 2* results[win -2][feat][i];
					}
				}
	...
	private void saveACEFeatureVectorsForARecording(
		...
		if (save_features_for_each_window)
			for (int win = 0; win < feature_values.length; win++) {
				double start_time = (window_start_indices[win])
						/ sampling_rate;
				double end_time = (window_start_indices[win]
						+ window_size - 1)
						/ sampling_rate;
